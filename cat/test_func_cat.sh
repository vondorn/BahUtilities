...
testing()
{
    ...
    ./s21_cat $t > test_bytes.log
    cat $t > test_poka.log
    DIFF_RES="$(diff -s test_bytes.log test_poka.log)"
    (( COUNTER++ ))
    if [ "$DIFF_RES" == "Files test_bytes.log and test_poka.log are identical" ]
    then
      (( SUCCESS++ ))
    ...
    rm test_bytes.log test_poka.log
}
